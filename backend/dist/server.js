...
server.listen(PORT, async () => {
    ...
    try {
        const { LiveSessionService } = await Promise.resolve().then(() => __importStar(require('./services/liveSessionService')));
        LiveSessionService.initializeCronJobs();
        console.log(`🔄 Live session status update cron job started`);
        logger_1.logger.info(`🔄 Live session status update cron job started`);
    }
    catch (error) {
