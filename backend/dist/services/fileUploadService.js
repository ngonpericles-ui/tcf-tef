...
class FileUploadService {
    ...
    static async getFileById(fileId, userId) {
        try {
            const file = await connection_1.prisma.file.findUnique({
                where: { id: fileId }
            });
            if (!file) {
    ...
    static async getUserFiles(userId, category, page = 1, limit = 20) {
        try {
            ...
            const [files, total] = await Promise.all([
                connection_1.prisma.file.findMany({
                    where: whereClause,
                    skip,
                    take: limit,
                    orderBy: { createdAt: 'desc' }
                }),
                connection_1.prisma.file.count({ where: whereClause })
            ]);
    ...
    static async deleteFile(fileId, userId) {
        try {
            ...
            await connection_1.prisma.file.delete({
                where: { id: fileId }
            });
            try {
                await promises_1.default.unlink(file.path);
    ...
    static async getFileStatistics(userId) {
        try {
            ...
            const [totalStats, categoryStats, mimetypeStats] = await Promise.all([
                connection_1.prisma.file.aggregate({
                    where: whereClause,
                    _count: { id: true },
                    _sum: { size: true }
                }),
                connection_1.prisma.file.groupBy({
                    by: ['category'],
                    where: whereClause,
                    _count: { id: true },
                    _sum: { size: true }
...
FileUploadService.MAX_FILE_SIZE = parseInt(process.env.MAX_FILE_SIZE || '0');
