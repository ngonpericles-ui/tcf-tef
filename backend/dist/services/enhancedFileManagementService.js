...
class EnhancedFileManagementService {
    static async uploadAndProcess(file, metadata) {
        try {
            ...
            const fileRecord = await connection_1.prisma.file.create({
                data: {
    ...
    static async searchFiles(filters, pagination) {
        try {
            ...
            const total = await connection_1.prisma.file.count({ where });
            const files = await connection_1.prisma.file.findMany({
                where,
