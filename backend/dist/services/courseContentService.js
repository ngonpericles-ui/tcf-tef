...
class CourseContentService {
    ...
    static async enrollInCourse(courseId, userId) {
        try {
            ...
            const enrollment = await connection_1.prisma.courseEnrollment.create({
                data: {
                    userId,
                    courseId,
                    enrolledAt: new Date()
    ...
    static async getUserEnrolledCourses(userId, page = 1, limit = 20) {
        try {
            ...
            const [enrollments, total] = await Promise.all([
                ...
                connection_1.prisma.courseEnrollment.count({ where: { userId } })
            ]);
...
class LessonService {
    static async createLesson(courseId, data, userId) {
        try {
            ...
            const existingLesson = await connection_1.prisma.lesson.findFirst({
                where: { courseId, order: data.order }
            });
            if (existingLesson) {
                throw new errors_1.ValidationError(`A lesson with order ${data.order} already exists`);
            }
            const lesson = await connection_1.prisma.lesson.create({
                data: {
                    title: data.title.trim(),
    ...
    static async markLessonCompleted(lessonId, userId) {
        try {
            const lesson = await connection_1.prisma.courseLesson.findUnique({
                where: { id: lessonId },
                include: {
                    course: {
                        select: {
            ...
            const existingCompletion = await connection_1.prisma.lessonCompletion.findUnique({
                where: {
                    userId_lessonId: {
                        userId,
                        lessonId
            ...
            await connection_1.prisma.lessonCompletion.create({
                data: {
                    userId,
                    lessonId,
            ...
            const courseLessons = await connection_1.prisma.courseLesson.findMany({
                where: { courseId: lesson.courseId },
                select: { id: true }
            });
            const completedLessons = await connection_1.prisma.lessonCompletion.count({
                where: {
                    userId,
    ...
    static async getCourseLessons(courseId) {
        try {
            ...
            const lessons = await connection_1.prisma.lesson.findMany({
                where: { courseId },
                orderBy: { order: 'asc' },
                select: {
                    id: true,
