...
class EmailService {
    static getLogoDataUri() {
        try {
            const possiblePaths = [
                path_1.default.join(process.cwd(), '../../frontend/public/logo/AURA.CA.png'),
                path_1.default.join(process.cwd(), '../frontend/public/logo/AURA.CA.png'),
                path_1.default.join(process.cwd(), 'frontend/public/logo/AURA.CA.png'),
                path_1.default.join(process.cwd(), 'public/logo/AURA.CA.png'),
                path_1.default.join(__dirname, '../../frontend/public/logo/AURA.CA.png'),
                path_1.default.join(__dirname, '../../../frontend/public/logo/AURA.CA.png'),
            ];
            ...
            logger_1.logger.warn('Logo file not found in any of the expected paths. Using text fallback.');
            return '';
        }
        catch (error) {
            logger_1.logger.error('Error loading logo for email', { error });
            return '';
        }
    }
    static wrapEmailWithProfessionalDesign(htmlContent) {
    ...
    static generateEmailContent(title, greeting, content, buttonText, buttonUrl, checklist) {
        let html = `
    ...
    static async sendWelcomeEmail(data) {
      ...
      <p style="margin-bottom: 1rem;"><strong>Conseil:</strong> Commencez par passer notre test de niveau pour obtenir un parcours personnalisé.</p>`;
        const html = this.generateEmailContent('Bienvenue sur AURA Learning Platform', `Bonjour ${data.firstName} ${data.lastName},`, content, 'Commencer maintenant', data.loginUrl);
    ...
    static async sendCourseEnrollmentEmail(data) {
      ...
      <p style="margin-top: 1rem;"><strong>Astuce:</strong> Activez les notifications pour ne manquer aucune mise à jour du cours.</p>`;
        const html = this.generateEmailContent(`Inscription confirmée: ${data.courseName}`, `Bonjour ${data.firstName},`, content, 'Accéder au cours', data.courseUrl);
        return this.sendEmail({
            to: data.email,
            subject,
            html: this.wrapEmailWithProfessionalDesign(html)
    ...
    static async sendLiveSessionReminderConfirmationEmail(data) {
      ...
      <p style="margin-top: 1rem;">Nous vous enverrons un rappel par email ${data.reminderMinutes || 5} minutes avant le début de la session pour ne rien manquer.</p>`;
        const html = this.generateEmailContent('Rappel : Vous avez ajouté un rappel', `Bonjour ${data.firstName},`, content);
        return this.sendEmail({
            to: data.email,
            subject,
            html: this.wrapEmailWithProfessionalDesign(html)
    ...
    static async sendLiveSessionReminderEmail(data) {
      ...
      <p style="margin-top: 1rem;">Rejoignez la session maintenant pour éviter tout problème technique.</p>`;
        ...
        const html = this.generateEmailContent(`Rappel : Session en direct "${data.sessionTitle}"`, `Bonjour ${data.firstName},`, content, 'Rejoindre la session', data.joinUrl, checklist);
        return this.sendEmail({
            to: data.email,
            subject,
            html: this.wrapEmailWithProfessionalDesign(html)
    ...
    static async sendTestResultsEmail(data) {
      ...
      <p style="margin-top: 1rem;">Continuez vos efforts! Chaque test vous rapproche de votre objectif.</p>`;
        const html = this.generateEmailContent(`Résultats de votre test: ${data.testName}`, `Bonjour ${data.firstName},`, content);
        return this.sendEmail({
            to: data.email,
            subject,
            html: this.wrapEmailWithProfessionalDesign(html)
    ...
    static async sendVoiceSimulationBookingEmail(data) {
      ...
      <p style="margin-top: 1rem;"><strong>Rappel:</strong> Vous recevrez un email de rappel 30 minutes avant votre simulation.</p>`;
        ...
        const html = this.generateEmailContent('Confirmation de votre simulation vocale TCF/TEF', `Bonjour ${data.firstName},`, content, data.accessUrl ? 'Accéder à la simulation' : undefined, data.accessUrl || undefined, checklist);
        try {
    ...
    static async sendVoiceSimulationReschedulingEmail(data) {
      ...
      <p style="margin-top: 1rem;">Si vous avez des questions ou besoin d'aide, n'hésitez pas à nous contacter.</p>`;
        const html = this.generateEmailContent('Confirmation de reprogrammation - Simulation vocale TCF/TEF', `Bonjour ${data.firstName},`, content, data.accessUrl ? 'Accéder à la simulation' : undefined, data.accessUrl || undefined);
        try {
            const result = await this.sendEmail({
                to: data.email,
                subject,
    ...
    static async sendVoiceSimulationReminderEmail(data) {
        ...
        <p style="margin-top: 1rem; margin-bottom: 1rem; color: #dc2626; font-weight: 600;">Ce lien est valide pendant 2 heures et à usage unique.</p>`;
            ...
            const html = this.generateEmailContent('Rappel : Votre simulation vocale débute bientôt', `Bonjour ${data.firstName},`, content, 'Commencer la simulation', directAccessLink, checklist);
            return this.sendEmail({
                to: data.email,
                subject,
                html: this.wrapEmailWithProfessionalDesign(html)
    ...
    static async sendVoiceSimulationResultsEmail(data) {
      ...
      <p style="margin-top: 1rem;">Continuez vos efforts, vous progressez bien!</p>`;
        const html = this.generateEmailContent('Résultats de votre simulation vocale TCF/TEF', `Bonjour ${data.firstName},`, content);
        return this.sendEmail({
            to: data.email,
            subject,
            html: this.wrapEmailWithProfessionalDesign(html)
    ...
    static async sendImmigrationSimulationReschedulingEmail(data) {
        try {
...
EmailService.fromAddress = `${process.env.EMAIL_FROM_NAME || 'AURA.CA'} <${process.env.EMAIL_FROM_ADDRESS || process.env.SMTP_USER || process.env.TWILIO_SMTP_USER || 'noreply@tcftef.com'}>`;
EmailService.replyToAddress = process.env.EMAIL_REPLY_TO || process.env.EMAIL_FROM_ADDRESS || 'periclesngon01@gmail.com';
