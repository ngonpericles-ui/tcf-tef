...
TestController.createTest = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const test = await testService_1.TestService.createTestWithQuestions(testData, questionsData, createdById, creatorRole);
    const response = {
        success: true,
        data: { test },
...
TestController.getTestResults = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { testId } = req.params;
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({
    ...
    const results = await testService_1.TestService.getTestResults(testId, userId);
    const response = {
        success: true,
        data: results,
