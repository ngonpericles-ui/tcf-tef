...
exports.testTokenGenerationAndExpiration = testTokenGenerationAndExpiration;
...
function testTokenGenerationAndExpiration() {
    ...
    const token = secureSessionService_1.SecureSessionService.generateSecureToken(sessionId, studentId);
    ...
    const validation = secureSessionService_1.SecureSessionService.validateSecureToken(token);
    if (validation.valid) {
    ...
    const sanitized = tokenWithBr
        ...
        .trim()
        .split('<br>')[0];
    const sanitizedValidation = secureSessionService_1.SecureSessionService.validateSecureToken(sanitized);
...
//# sourceMappingURL=test-token-validation.js.map