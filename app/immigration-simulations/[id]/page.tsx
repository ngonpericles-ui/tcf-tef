...
function ImmigrationSimulationRoomContent() {
  ...
  const [simulation, setSimulation] = useState<ImmigrationSimulation | null>(null);
  ...
  useEffect(() => {
    const loadSimulation = async () => {
      try {
        const url = `/api/immigration-simulation/${simulationId}`;
        ...
        if (!response.ok) {
          const errorResponse = await response.json().catch(() => ({ message: 'Server error' }));
          ...
          if (errorResponse.code === 'TOO_EARLY') {
            ...
            setError(errorResponse.message || t_('Access denied or simulation not accessible', 'Access denied or simulation not accessible'));
        ...
        if (data.data.status === 'COMPLETED' || data.data.status === 'FINISHED') {
          const endedAt = data.data.endedAt || data.data.completedAt;
          if (endedAt) {
        ...
        // Check simulation accessibility
        ...
        // Check if simulation is accessible (5 minutes before or less)
        if (minutesUntilStart > 5) {
          ...
          // Redirect to guide if student hasn't seen it yet
          const guideSeen = localStorage.getItem(`guide_seen_immigration_${simulationId}`);
          if (!guideSeen) {
            const guideUrl = token
              ? `/immigration-simulations/${simulationId}/guide?token=${token}`
              : `/immigration-simulations/${simulationId}/guide`;
    ...
    if (isCallActive) {
      timerRef.current = setInterval(() => {
        setCallDuration(prev => {
          ...
          // Stop after simulation duration (5 minutes = 300 seconds)
  ...
  const handleStartCall = async () => {
    try {
      const startUrl = token
        ? `/api/immigration-simulation/start/${simulationId}?token=${token}`
      ...
      const response = await fetch(startUrl, {
        ...
        headers: {
          'Content-Type': 'application/json',
          ...(token ? {} : { 'Authorization': `Bearer ${localStorage.getItem('token') || localStorage.getItem('access_token')}` })
        }
      });
  ...
  const handleEndCall = async () => {
    try {
      const endUrl = token
        ? `/api/immigration-simulation/finish/${simulationId}?token=${token}`
        : `/api/immigration-simulation/finish/${simulationId}`;
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          router.push(`/immigration-simulations/results?id=${simulationId}`);
        }, 2000);
      } else {
  ...
  // Circular Timer Component - No numbers, green by default, red at 30s
  const CircularTimer = ({ currentTime, totalTime }: { currentTime: number; totalTime: number }) => {
    ...
    const isUrgent = remaining <= 30; // Red when 30 seconds or less remain
    const size = 120;
    const strokeWidth = 8;
      ...
      >
        <svg width={size} height={size} className="transform -rotate-90">
          ...
          <motion.circle
            ...
            cy={size / 2}
            r={radius}
            stroke={isUrgent ? '#ef4444' : '#10b981'} // Red if urgent, green otherwise
            strokeWidth={strokeWidth}
            fill="none"
            strokeLinecap="round"
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">{t_('Chargement de la simulation...', 'Loading simulation...')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full dark:bg-gray-900/50 dark:border-gray-800">
          <CardContent className="pt-6 text-center">
            ...
            <Button onClick={() => router.push('/immigration-simulations/results?id=' + simulationId)} className="mr-2">
              {t_('Voir les Résultats', 'View Results')}
            </Button>
            <Button variant="outline" onClick={() => router.push('/immigration-simulations')}>
              {t_('Retour', 'Back')}
            </Button>
          </CardContent>
        </Card>
  ...
  // Accessible if:
  // - 5 minutes or less before start (minutesUntilStart between 0 and 5)
  // - During simulation (after start but before end)
  const canStart = (minutesUntilStart <= 5 && minutesUntilStart >= 0) || (now >= scheduledDate && now <= simulationEnd);

  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      <div className="flex flex-col h-screen">
        {/* Student interface - Main camera */}
        <div className="flex-1 relative bg-black">
          ...
          {/* Message if not yet accessible */}
          {!canStart && !isCallActive && (
            <div className="absolute inset-0 bg-black/80 flex items-center justify-center">
              <Card className="max-w-md dark:bg-gray-900/50 dark:border-gray-800">
                <CardContent className="pt-6 text-center">
                  <Loader2 className="h-12 w-12 animate-spin text-blue-600 mx-auto mb-4" />
        ...
        {/* Controls - Hang Call, Microphone, Camera */}
        <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-t border-gray-200 dark:border-gray-800 p-6">
          <div className="max-w-4xl mx-auto flex items-center justify-center gap-4">
            ...
            <Button
              ...
              className="h-16 w-16 rounded-full dark:border-gray-700"
          ...
          {!isCallActive && canStart && (
            <p className="text-center text-sm text-gray-600 dark:text-gray-400 mt-4">
              {t_('Cliquez sur le bouton vert pour démarrer la simulation d\'immigration', 'Click the green button to start the immigration simulation')}
            </p>
          )}
        </div>
      </div>
...
export default function ImmigrationSimulationRoomPage() {
  return (
    <SharedDataProvider>
      <ImmigrationSimulationRoomContent />
    </SharedDataProvider>
  );
}
