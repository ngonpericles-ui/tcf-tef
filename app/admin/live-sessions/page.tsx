...
export default function AdminSessionsPage({ role: propRole }: ManagerSessionsPageProps = {}) {
  ...
  useEffect(() => {
    ...
    // Set current manager from user data (Admin has full permissions)
    const manager = {
      role: 'admin' as const, // Always admin for this page
      name: `${user.firstName || ''} ${user.lastName || ''}`.trim() || user.email?.split('@')[0] || 'Admin',
      permissions: {
        hostLiveSessions: true,
        createCourses: true,
        createTests: true, // Admin has all permissions
      },
    }
    ...
    const fetchProPlusStudents = async () => {
      ...
      try {
        ...
        const response = await apiClient.get('/admin/users', {
          params: { role: 'STUDENT', subscriptionTier: 'PRO' }
        })
