...
export default function SocialAuthButtons({ 
  ...
  const handleSocialSignIn = async (provider: 'google' | 'apple', idToken: string) => {
    try {
      ...
      if (data.success) {
        ...
        await login(data.data.user.email || '', '') // Empty password for social auth
        
        toast({
          title: "Success!",
          description: `Signed in with ${provider === 'google' ? 'Google' : 'Apple'}`,
        })
  ...
  const handleAppleSignIn = async () => {
    try {
      ...
      await handleSocialSignIn('apple', idToken)
    } catch (error: any) {
      if (error.code !== 'auth/popup-closed-by-user') {
